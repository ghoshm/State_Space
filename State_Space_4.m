...
p_assign_dist = nan(size(idx{numComp,1},1),1,'single'); % pre-allocate 

for o = 1:size(p_assign_dist,1) % For each observation 
    p_assign_dist(o,1) = single(P{numComp,1}(o,idx{numComp,1}(o,1))); % Pull posterior probabilities
    % For each cluster assignment
end

figure; 
histogram(p_assign_dist,'binwidth',0.1)
...
%% "Goodness" of fit for each cluster
p_assign_dist_mean = nan(max(fish_tags{1,1}),numComp,'single'); % pre-allocate  
    % fish x clusters 

for f = 1:max(fish_tags{1,1}) % For each fish  
    for c = 1:numComp % For each cluster 
        p_assign_dist_mean(f,c) = nanmean(p_assign_dist(idx_numComp_sorted == c...
            & fish_tags{1,1} == f));  
    end 
end 
...
for c = 1:numComp % For each cluster
    subplot(2,5,c); title(horzcat('Cluster ',num2str(c)));
    for e = 1:max(experiment_tags{1,1}) % For each experiment
        spread_cols = plotSpread(p_assign_dist_mean(i_experiment_tags == e,c),...
...
for c = 1:numComp % For each cluster
    if max(experiment_tags{1,1}) > 1 % With experiment tags 
        [twa.gf.p(1:3,c),~,twa.gf.stats{c}] = anovan(p_assign_dist_mean(:,c),...
            {i_group_tags,i_experiment_tags},'display','off','model','full');
    else % Without experiment tags 
        [twa.gf.p(1,c),~,twa.gf.stats{c}] = anovan(p_assign_dist_mean(:,c),...
            {i_group_tags},'display','off','model','full'); % Try without
    end
end

clear c e f spread_cols
%% Bout Proportions  
bout_proportions = nan(max(fish_tags{1,1}),numComp,max(parameter_indicies{1,1}),...
    'single'); % fish x clusters x time windows 

for f = 1:max(fish_tags{1,1}) % For each fish 
    for c = 1:numComp % For each bout type 
        for t = 1:max(parameter_indicies{1,1}) % For each time window 
            bout_proportions(f,c,t) = size(find(fish_tags{1,1} == f & ...
                idx_numComp_sorted == c & parameter_indicies{1,1} == t),1)/...
                    size(find(fish_tags{1,1} == f & parameter_indicies{1,1} == t),1); 
        end 
    end 
    disp(horzcat('Calculated Bout Proportions for fish ',num2str(f),...
...
figure; hold on;
for c = 1:numComp % for each cluster
    subplot(2,5,c); title(horzcat('Cluster ',num2str(c))); hold on;
    counter = 1; clear scrap; % subplot
    for e = 1:max(i_experiment_tags) % for each experiment
        for g = 1:max(i_group_tags) % for each group
            legend_lines(g) = errorbar(nanmean(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2])),...
                nanstd(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2]))/...
                sqrt(size(permute(bout_proportions(i_group_tags == g &...
            ...
            scrap(1,counter) = max(nanmean(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2])) +...
                nanstd(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2]))/...
                sqrt(size(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2]),1)));
            scrap(2,counter) = min(nanmean(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2])) -...
                nanstd(permute(bout_proportions(i_group_tags == g &...
                i_experiment_tags == e,c,time_window(1):time_window(2)),[1 3 2]))/...
                sqrt(size(permute(bout_proportions(i_group_tags == g &...
...
clear c counter scrap e g a y_lims r legend_cell legend_cols legend_lines
...
for c = 1:numComp % For each cluster 
    clear scrap;
    scrap = permute(bout_proportions(:,c,time_window(1):time_window(2)),[1 3 2]); 
    ...
    if size(days_crop(days),2) == size(nights_crop(nights),2) % If comparing development 
        ...
        if max(experiment_tags{1,1}) > 1 % With experiments 
            ...
            [twa.bp.p(1:3,c),~,twa.bp.stats{c}] = anovan(scrap,...
...
% Cluster values for each parameter (need to correct units)  
figure; hold on; 
for p = 1:6 % For each parameter 
    subplot(2,3,p); title(parameters{p}); 
    plotSpread(wake_cells(:,p+2)/unit_conversion(1,p),'distributionIdx',idx_numComp_sorted,...
        'distributionColors',cmap_cluster); 
    set(gca,'xtick',[]); 
...
figure; hold on; 
for c = 1:numComp  % For each cluster 
    scatter(score(idx_numComp_sorted==c,1),score(idx_numComp_sorted==c,2),...
...
figure; hold on; 
for c = 1:numComp % For each cluster 
    plot(nanmean(X(idx_numComp_sorted==c,:)),'color',cmap_cluster(c,:),'linewidth',3);
    set(gca,'Xtick',1:6);
